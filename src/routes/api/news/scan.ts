...
import { runNewsDigestScan } from "@/lib/news/digest-scan";
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          const report = await runNewsDigestScan({ force });
          return json({ ok: true, ...report });
        } catch (err) {
          reportServerError(err, { route: "/api/news/scan" });
          const message = err instanceof Error ? err.message : "Falha no scan de notícias.";
