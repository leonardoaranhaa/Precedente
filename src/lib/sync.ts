...
export async function getSyncDataFor(userId: string, kind: SyncKind): Promise<Json> {
  const sql = await getSql();
  const rows = await sql.query<{ data: Json }>(
...
export const getSyncData = createServerFn({ method: "GET" })
  ...
  .handler(({ context, data: kind }) => getSyncDataFor(context.userId, kind));
...
export const setSyncData = createServerFn({ method: "POST" })
  ...
  .middleware([authMiddleware])
  .handler(({ context, data: input }) => setSyncDataFor(context.userId, input.kind, input.data));
