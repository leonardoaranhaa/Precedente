...
/** Respostas curtas, só com campos já calculados — sem LLM. */
export function answerAssistantQuestion(
  ...
  if (o.liquidityUsd != null) {
    bits.push(`Liquidez DEX ≈ ${formatUsd(o.liquidityUsd)}.`);
  }
  ...
  return (
    bits.join(" ") +
    " Contexto de pressão e profundidade — não ordem de direção."
  );
}
